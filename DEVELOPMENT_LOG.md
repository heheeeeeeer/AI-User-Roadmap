...
## 2026-07-10（修复移动端详情页被遮罩覆盖）

### 改动摘要

- 修复移动端点击阶段后只显示灰色遮罩、不显示详情内容的问题。
- 根因是移动端零尺寸 `.detail-dock` 仍继承了桌面端打开态的 `transform`，导致它成为 fixed 详情页的包含块和层叠上下文，详情页因此被后置遮罩压住。
- 在 900px 及以下断点明确清除详情容器的 transform、宽度占位、外边距和桌面端过渡，使全屏详情页重新相对视口定位并正确显示在遮罩之上。
...
- 在 390 × 844 浏览器移动端视口点击“前言”，确认详情页尺寸为 390 × 844、位置为 `(0, 0)`、不透明度为 1，z-index 50 高于遮罩的 z-index 40，内容正常可见。
- 确认移动端打开详情后 body 滚动正常锁定，关闭后 body 定位、根元素 overflow 与抽屉状态均正常恢复。
- 确认 390px 移动端与 1440px 桌面端均无横向溢出；桌面端详情侧栏仍保持距视口上下各 18px。
- `git diff --check` 检查通过。

### 下一次改动前需要防止复现

- 不要在移动端 `.detail-dock` 或它的打开态上恢复 `transform`，否则内部 `position: fixed` 详情页会再次失去视口定位与正确层级。
- 继续保持桌面端详情侧栏的固定宽度、上下间距和无文字重排动画，并回归移动端关闭、Escape、滚动锁定及无横向溢出。
...
- 收窄 901–1120px 桌面断点下的右侧详情面板宽度，从偏大的 `clamp(500px, 54vw, 560px)` 调整为更保守的 `clamp(340px, 42vw, 460px)`，避免 DevTools 打开或窗口变窄时右侧面板把页面撑出横向滚动。
- 同步降低该断点下左侧路线图的打开态 flex basis，让左右两侧在中窄宽度下可以自然共存。
- 为详情面板内容、overview、资源卡标题和说明补充 `min-width: 0` 与 `overflow-wrap: anywhere`，允许长标题、英文词组和中文长句自然换行，不再撑出横向滚动。
- 清理 `src/data/roadmap.js` 中一处行尾空格，避免 `git diff --check` 失败。
...
- `npm run build` 构建通过，Astro check 为 0 errors、0 warnings、0 hints。
- 本地预览 `http://127.0.0.1:8766/` 返回 `HTTP/1.1 200 OK`。
- 在 1008px 宽度下初步检查，页面级 `scrollWidth === clientWidth`；后续定位到右侧详情面板内部 overview 曾存在轻微横向溢出，并已针对该区域补充收缩与换行规则。
- 尝试继续用浏览器工具复测 1008px 详情面板内部滚动宽度，但浏览器视口控制连续超时；最终以 CSS 约束、构建和 whitespace 检查作为本轮验证依据。

### 下一次改动前需要防止复现

- 901–1120px 宽度下不要重新把详情面板宽度调回 500px 以上，否则 DevTools 打开或窄桌面会再次产生横向滚动。
- 详情面板内部的 overview、资源卡标题和说明需要允许换行，不能用固定宽度或不可收缩网格项撑开容器。
- 保持桌面详情面板上下边距固定、移动端全屏抽屉、核心场景二级页返回入口和资源默认可见。

## 2026-07-05（固定桌面详情面板上下边距）

### 改动摘要

- 将桌面端打开详情时的右侧详情容器从 `sticky` 调整为相对视口固定定位，避免外部页面滚到顶部或底部时，详情面板上下留白跟随页面边界漂移。
- 为打开状态增加桌面端布局占位，保留原本左侧路线图被详情面板挤开的视觉关系，不让固定定位导致左侧内容突然变宽。
- 抽出详情面板宽度、间距和页面内边距变量，继续兼容 901–1120px 的中窄桌面布局。
...
- `git diff --check` 检查通过。
- 本地预览 `http://127.0.0.1:8766/` 返回 `HTTP/1.1 200 OK`。
- 在 1280px 宽度下打开 `Agent` 详情面板，并在左侧路线图区域外部滚动到页面顶部和底部，确认右侧详情容器保持 `position: fixed`，顶部间距与底部间距均稳定为 18px，页面无横向溢出。
- 尝试继续用浏览器工具切换 1008px 与 390px 视口回归，但视口切换过程超时；代码层面已将固定定位限制在 `min-width: 901px`，移动端仍沿用原本 fixed 全屏抽屉规则。

### 下一次改动前需要防止复现

- 桌面端右侧详情面板打开后，外部页面滚动到顶部或底部时，面板上下留白不能再漂移。
- 不要移除 `.product-workspace.is-detail-open::after` 的布局占位，否则桌面端详情面板固定定位后左侧路线图可能重新铺满宽度。
- 移动端仍应保持全屏抽屉，不要套用桌面端右侧固定卡片规则。
- 继续保护核心场景二级页返回入口、无横向溢出和资源默认可见。

## 2026-07-05（核心场景二级页返回入口）

### 改动摘要

- 为 `roadmap-data.json` 中的二级 panel 增加 `parent` 信息，当前「核心场景」下的二级页面会记录父级 `intro-panel-core`。
- 在二级详情页标题上方渲染轻量返回入口 `‹ 核心场景`。
- 抽离按 panel id 打开详情页的逻辑，让返回入口可以复用现有面板切换、滚动复位、移动端锁滚动和高亮逻辑。
- 返回父级时保持详情面板打开，并继续高亮左侧「核心场景」主阶段；右上角关闭和 Escape 仍关闭整个详情面板。
...
- `npm run build` 构建通过，Astro check 为 0 errors、0 warnings、0 hints。
- `git diff --check` 检查通过。
- 读取构建后的 `/roadmap-data.json` 结构，确认「信息调研」等二级 panel 带有 `parent.id = "intro-panel-core"` 与 `parent.title = "核心场景"`。
- 使用源码搜索确认无父级的一级页面不会渲染 `.panel-back-button`。

### 下一次改动前需要防止复现

- 二级页面返回应只返回父级目录页，不要关闭整个详情面板。
- 返回父级后左侧主路线图应继续高亮父级阶段。
- 不要引入 URL hash 或浏览器 history，避免打断当前轻量抽屉交互。
- 继续保护移动端抽屉滚动锁定、无横向溢出和核心场景二级入口可用。
